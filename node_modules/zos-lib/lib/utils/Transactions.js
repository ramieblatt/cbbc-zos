 ...
 */
async function sendTransaction(contractFn, args = [], txParams = {}) {
  ...
  return contractFn(...args, _extends({ gas: gasToUse }, txParams));
}
...
async function deploy(contract, args = [], txParams = {}) {
  ...
  const gasToUse = await calculateActualGas(estimatedGas);
  return contract.new(...args, _extends({ gas: gasToUse }, txParams));
}

async function estimateGas(txData, txParams) {
  // Use json-rpc method estimateGas to retrieve estimated value
  return (0, _util.promisify)(web3.eth.estimateGas.bind(web3.eth))(_extends({ data: txData }, txParams));
}
